"""
This script evaluates all pairs of features combined with subject class
information using group-aware cross-validation and parallel execution.
"""
...
def zscore_by_subject_using_baseline(
    ...
    """
    Normalize features per subject using background as baseline.
    """
    ...
    if verbose:
        print(f"Normalized features: {len(feats)}")
...
def split_sessions_within_name(
    ...
    """
    Session-level split preserving subject independence.
    """
...
OUTDIR = "/storage5/ame.lia/TUSZ-BIDS1/svm_pairwise2"

os.makedirs(OUTDIR, exist_ok=True)

os.makedirs(os.path.join(OUTDIR, "models_all"), exist_ok=True)


def eval_one_pair(f1, f2):

    x_cols = [f1, f2, "new_class"]
...
summary_df.to_csv(
    os.path.join(OUTDIR, "summary_pairwise_svm.csv"),
    index=False,
)
...
for i, r in enumerate(ok_df.itertuples(index=False), start=1):
    ...
    slug = _safe_slug(f"{f1}__{f2}")

    path = os.path.join(
        OUTDIR,
        "models_all",
        f"svm_{i:05d}__{slug}.joblib",
    )
...
pd.DataFrame(models_rows).to_csv(
    os.path.join(
        OUTDIR,
        "all_pairwise_svm_models.csv"
    ),
    index=False,
)
